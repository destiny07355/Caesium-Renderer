...
public final class SpriteAnimationController {
    ...
    public static boolean shouldSkipFast(CaesiumSpriteAnimator animator) {
        ...
        // Fire is a correctness-sensitive animation. When both animation controls are
        // enabled, never let the optional visibility/slow-frame policy freeze it.
        if ((cat & SpriteVisibilityTracker.CAT_FIRE) != 0) {
            RendererConfig config = RendererConfig.get();
            if (config.enableTextureAnimations && config.enableFireAnim) return false;
        }
