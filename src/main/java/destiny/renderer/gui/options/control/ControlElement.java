 ...
 * Features a clear title and concise secondary description,
...
public abstract class ControlElement<T> extends ClickableWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int titleY = height <= 26 ? (getY() + (height - 8) / 2) : (getY() + 9);
        context.drawText(tr, CaesiumFont.text(title), getX() + CaesiumTheme.ITEM_INDENT,
            titleY, titleColor, false);

        if (height > 26) {
            String description = option.getTooltip() == null ? "" : option.getTooltip().getString();
            if (!description.isEmpty()) {
                int descriptionMaxW = Math.max(10, labelMaxW);
                if (tr.getWidth(description) > descriptionMaxW) {
                    description = tr.trimToWidth(description,
                        Math.max(10, descriptionMaxW - tr.getWidth("..."))) + "...";
                }
                context.drawText(tr, CaesiumFont.text(description),
                    getX() + CaesiumTheme.ITEM_INDENT, getY() + 24,
